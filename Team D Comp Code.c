...
#pragma config(Motor,  port2,           leftFront,     tmotorVex393_MC29, openLoop, encoderPort, I2C_2)
#pragma config(Motor,  port3,           backRight,     tmotorVex393_MC29, openLoop, reversed)
#pragma config(Motor,  port4,           rightFront,    tmotorVex393_MC29, openLoop, reversed, encoderPort, I2C_1)
...
*/
	...
	if(autonColorChooser) {
		writeDebugStreamLine("AUTON - BLUE");
		if(autonTileChooser) {
			writeDebugStreamLine("AUTON - RIGHT");
			if(pidRunTimes == 0) {
				startTask(shootBall);
				wait1Msec(3500);
				...
				driveToAngle(90, true); //turn 90 degrees to the right
			} else if(pidTaskEnded && pidRunTimes == 2) {
				writeDebugStreamLine("COMMAND --- 2 --- FINISHED");
				driveForwardToPosition(-3, -3);
				...
				writeDebugStreamLine("AUTON - LEFT");
				if(pidRunTimes == 0) {
					startTask(moveFlipper);
					driveForwardToPosition(3,3);
				} else if(pidTaskEnded && pidRunTimes == 1) {
					writeDebugStreamLine("COMMAND --- 1 --- FINISHED");
					driveToAngle(90, false); //turn 90 degrees to the left
				} else if(pidTaskEnded && pidRunTimes == 2) {
					writeDebugStreamLine("COMMAND --- 2 --- FINISHED");
					driveForwardToPosition(4.5,4.5);
				} else if(pidTaskEnded && pidRunTimes == 3) {
					writeDebugStreamLine("COMMAND --- 3 --- FINISHED");
					driveForwardToPosition(-3,-3);
				} else if(pidTaskEnded && pidRunTimes == 4) {
					writeDebugStreamLine("COMMAND --- 4 --- FINISHED");
					stopDrive();
					writeDebugStreamLine("ENDED");
			}
		}
...
task usercontrol() {
	...
	while(true) {
		if((abs(vexRT[Ch2]) >= 0.2) && (abs(vexRT[Ch3]) >= 0.2)) {
