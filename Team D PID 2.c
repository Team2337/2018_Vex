...
#pragma config(Sensor, I2C_3,  clawEncoder,    sensorQuadEncoderOnI2CPort,    , AutoAssign )
...
#pragma config(Motor,  port8,           claw,          tmotorVex393_MC29, openLoop, encoderPort, I2C_3)
...
task moveArm() {
	while(true) {
		writeDebugStreamLine("Claw Encoder: %d", SensorValue(clawEncoder));
		if(armPosition == "DOWN") {
			if(SensorValue(clawEncoder) >= 0) {
			...
			} else if(SensorValue(clawEncoder) <= 1000) {
